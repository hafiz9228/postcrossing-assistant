...
def encode_image_ref_to_base64(image_ref: str) -> str:
    if not image_ref:
        raise ValueError("Missing image reference.")
    image_url = get_public_image_url(image_ref)
...
def delete_postcard_by_id(postcard_id):
    ...
    supabase = get_supabase()
    image_key = postcard.get("image_path")

    supabase.table("postcards").delete().eq("id", postcard_id).execute()

    if image_key:
        try:
            supabase.storage.from_(SUPABASE_BUCKET).remove([image_key])
        except Exception:
            pass
...
def generate_next_code(theme_abbr):
    ...
    codes = [r.get("code") for r in (res.data or []) if r.get("code")]
    if not codes:
...
def update_postcard_from_analysis(postcard_id, description, theme, tags):
    ...
    supabase.table("postcards").update({
        ...
        "tags": "; ".join(tags),
        "status": "in_stock",
        "notes": "Updated by AI analysis in UI",
        "date_added": now_kl().isoformat(),
...
def fetch_theme_stats():
    supabase = get_supabase()
    res = supabase.table("postcards").select("theme, status").execute()
    ...
    for row in (res.data or []):
        ...
        stats[theme]["total"] += 1
        if status in stats[theme]:
            stats[theme][status] += 1

    rows = []
    for theme in sorted(stats.keys()):
        s = stats[theme]
        rows.append((
            theme,
            s["total"],
...
def get_request_theme_signals():
    supabase = get_supabase()
    res = supabase.table("request_logs").select("extracted_preferences").execute()
    ...
    for row in (res.data or []):
        prefs = row.get("extracted_preferences")
        if not prefs:
            continue
...
def analyze_image_with_llm(image_ref):
    if not LLM_API_KEY:
    ...
    image_b64 = encode_image_ref_to_base64(image_ref)
...
with tabs[0]:
    ...
    draft_options = {
        f"ID {row[0]} | {Path(str(row[1])).name}": row[0]
        for row in drafts
    }
    ...
    if draft_options:
        ...
        if postcard and postcard["image_path"]:
            st.image(
                get_public_image_url(postcard["image_path"]),
                caption=f"Draft ID {selected_id}",
                width=350,
            )
        ...
        with col1:
            if st.button("Analyze Draft with AI"):
                try:
                    options = analyze_image_with_llm(postcard["image_path"])
    ...
    if "analysis_options" in st.session_state:
        current_id = st.session_state.get("analysis_postcard_id")
        if current_id == draft_options.get(selected_label) if draft_options else None:
    ...
    if "smart_preferences" in st.session_state and "smart_shortlist" in st.session_state:
        ...
        for i, item in enumerate(shortlist, start=1):
            ...
            if postcard["image_path"]:
                st.image(get_public_image_url(postcard["image_path"]), width=220)
        ...
        if not reranked:
            ...
            for item in reranked:
                ...
                if postcard["image_path"]:
                    st.image(get_public_image_url(postcard["image_path"]), width=260)
...
with tabs[2]:
    ...
    if send_code.strip():
        ...
        if postcard_preview:
            ...
            if postcard_preview["image_path"]:
                st.image(get_public_image_url(postcard_preview["image_path"]), width=220)
...
with tabs[3]:
    st.header("Stock")
    st.info("Backup disabled in Supabase version.")
...
with tabs[4]:
    ...
    if not rows:
        ...
        if not filtered_postcards:
            ...
            with col_left:
                if selected_postcard["image_path"]:
                    st.image(get_public_image_url(selected_postcard["image_path"]), width=280)
                else:
